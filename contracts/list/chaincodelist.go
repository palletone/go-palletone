...
func SetChaincode(cid string, version int, chaincode *CCInfo) error {
	...
	log.Debug("SetChaincode", "chainId", cid, "cVersion", version, "Name", chaincode.Name,
		"chaincode.version", chaincode.Version, "Id", chaincode.Id)
	for k, v := range chains.Clist {
		if k == cid {
			log.Debug("SetChaincode, chainId already exit", "cid:", cid, "version", version)
