...
import (
	...
	mocklgr "github.com/palletone/go-palletone/core/vmContractPub/mocks/ledger"
...
//glh
	...
	viper.Set("chaincode.executetimeout", 5000000000) //5s
...
func startTx(t *testing.T, chainID string, cis *pb.ChaincodeInvocationSpec, txId string) (context.Context, int , *pb.SignedProposal, *pb.Proposal) {
	ctxt := context.Background()
	...
	//	t.Fatalf("getting txsimulator failed %s", err)
	//}
	return ctxt, 0, nil, nil
}

func endTx(t *testing.T, cccid *ccprovider.CCContext, txsim int, cis *pb.ChaincodeInvocationSpec) {
...
func TestLaunchAndWaitTimeout(t *testing.T) {
	newCCSupport := &ChaincodeSupport{peerTLS: false, chaincodeLogLevel: "debug", shimLogLevel: "info", ccStartupTimeout: time.Duration(5000) * time.Millisecond, runningChaincodes: &runningChaincodes{chaincodeMap: make(map[string]*chaincodeRTEnv), launchStarted: make(map[string]bool)}, peerNetworkID: "networkID", peerID: "peerID"}
	spec := &pb.ChaincodeSpec{Type: pb.ChaincodeSpec_Type(pb.ChaincodeSpec_Type_value["GOLANG"]), ChaincodeId: &pb.ChaincodeID{Name: "testcc", Version: "0"}}
	code := getTarGZ(t, "/home/glh/tmp/abc.go", []byte("code"))
	cds := &pb.ChaincodeDeploymentSpec{ChaincodeSpec: spec, CodePackage: code}
	cccid := ccprovider.NewCCContext("testchannel", "testcc", "0", "landwtimertest_txid", false, nil, nil)
