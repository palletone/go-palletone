...
func initPeer(chainIDs ...string) (net.Listener, error) {
	...
	//	if err = MockCreateChain(id); err != nil {
	...
	//		fmt.Printf("glh:GetTestChainID not equal id\n")
	//	}
	//}
...
*/
	...
	fmt.Println("begin to PutChaincodeIntoFS")
	...
	// //sprop, prop := MockSignedEndorserProposal2OrPanic(cccid.ChainID, cis.ChaincodeSpec, signer)
	//sprop, prop := MockSignedEndorserProposal2OrPanic(cccid.ChainID, cis.ChaincodeSpec)
	...
	//}
	fmt.Println("begin to ExecuteWithErrorFilter")
...
func invokeExample02Transaction(ctxt context.Context, cccid *ccprovider.CCContext, cID *pb.ChaincodeID, chaincodeType pb.ChaincodeSpec_Type, args []string, destroyImage bool) error {
	...
	fmt.Println("invokeExample02Transaction, begin init")
	f := "init"
	...
	fmt.Println("invokeExample02Transaction, begin invoke")
	f = "invoke"
	...
	fmt.Println("invokeExample02Transaction, begin delete")
...
func TestExecuteDeployTransaction(t *testing.T) {
	fmt.Println("TestExecuteDeployTransaction enter!")
	//chaincoe is deployed as part of many tests. No need for a separate one for this
	//t.Skip()

	chainID := util.GetTestChainID()

	executeDeployTransaction(t, chainID, "example01", "chaincode/example01")
}
...
func TestExecuteInvokeTransaction(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.chaincodeType.String(), func(t *testing.T) {
			...
			args := []string{"a", "b", "10"}
			err = invokeExample02Transaction(ctxt, cccid, ccID, tc.chaincodeType, args, false) //true
			if err != nil {
				t.Fail()
...
func TestChaincodeInvokeChaincodeErrorCase(t *testing.T) {
	...
	sProp, prop := MockSignedEndorserProposalOrPanic(util.GetTestChainID(), spec1, []byte([]byte("Alice")), nil)
...
func setupTestConfig() {
	...
	viper.SetConfigName("chaincodetest.yaml") // name of config file (without extension)
	viper.AddConfigPath("/home/glh/tmp/config/")            // path to look for the config file in
...
func deployChaincode(ctx context.Context, name string, version string, chaincodeType pb.ChaincodeSpec_Type, path string, args [][]byte, creator []byte, channel string, nextBlockNumber uint64) ([]byte, *ccprovider.CCContext, error) {
	...
	//glh
	//signedProposal, proposal := MockSignedEndorserProposal2OrPanic(channel, chaincodeSpec, signer)
	signedProposal, proposal := MockSignedEndorserProposal2OrPanic(channel, chaincodeSpec)
