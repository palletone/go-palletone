...
contract PTNETH is IERC20 {
    ...
    string constant public name = "PTN connect ETH";
    string constant public symbol = "PTNETH";
    ...
    function encodeBase58Test(address addrHex) constant returns (string) {
        uint160 a = uint160(addrHex);
        string memory result = bytesConcat(toBase58(toBytes(a)));
        return result;
    }
    ...
    function withdraw(address recver, uint amount, bytes32 reqid, bytes sigstr1, bytes sigstr2, bytes sigstr3) public {
      ...
      ptnToken.transfer(recver,amount);
      emit Withdraw(ptnToken, msg.sender, recver, amount, reqid);
    }
    ...
    function() payable {//TODO test event exist or not
      emit Deposit(ptnToken, msg.sender, msg.value);
    }
}
