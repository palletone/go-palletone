...
type Handler struct {
	...
	responseChannel map[string]chan pb.PtnChaincodeMessage
...
func (handler *Handler) createChannel(channelID, txid string) (chan pb.PtnChaincodeMessage, error) {
	handler.Lock()
	...
	c := make(chan pb.PtnChaincodeMessage)
	handler.responseChannel[txCtxID] = c
	return c, nil
}

func (handler *Handler) sendChannel(msg *pb.PtnChaincodeMessage) error {
	handler.Lock()
	defer handler.Unlock()
	if handler.responseChannel == nil {
...
func (handler *Handler) sendReceive(msg *pb.PtnChaincodeMessage, c chan pb.PtnChaincodeMessage) (pb.PtnChaincodeMessage,
	error) {
	...
	for {
		...
		case err := <-errc:
			...
			return pb.PtnChaincodeMessage{}, err
		case outmsg, val := <-c:
			if !val {
				return pb.PtnChaincodeMessage{}, errors.New("unexpected failure on receive")
			}
			return outmsg, nil
		}
	}
...
func (handler *Handler) beforeRegistered(e *fsm.Event) {
	if _, ok := e.Args[0].(*pb.PtnChaincodeMessage); !ok {
		e.Cancel(errors.New("Received unexpected message type"))
		return
	}
	log.Debugf("Received %s, ready for invocations", pb.PtnChaincodeMessage_REGISTERED)
}

// handleInit handles request to initialize chaincode.
func (handler *Handler) handleInit(msg *pb.PtnChaincodeMessage) {
	// The defer followed by triggering a go routine dance is needed to ensure that the previous state transition
	// is completed before the next one is triggered. The previous state transition is deemed complete only when
	// the beforeInit function is exited. Interesting bug fix!!
	go func() {
		var nextStateMsg *pb.PtnChaincodeMessage
		...
		errFunc := func(err error, payload []byte, ce *pb.PtnChaincodeEvent, errFmt string,
			args ...interface{}) *pb.PtnChaincodeMessage {
			if err != nil {
				...
				return &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_ERROR, Payload: payload, Txid: msg.Txid,
		...
		if nextStateMsg = errFunc(unmarshalErr, nil, nil, "[%s]Incorrect payload format. "+
			"Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_ERROR.String()); nextStateMsg != nil {
			return
		}
		...
		if res.Status >= ERROR {
			...
			if nextStateMsg = errFunc(err, []byte(res.Message), stub.chaincodeEvent, "[%s]Init get error "+
				"response. Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_ERROR.String()); nextStateMsg != nil {
				return
			}
		}
		...
		nextStateMsg = &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_COMPLETED, Payload: resBytes, Txid: msg.Txid,
			ChaincodeEvent: stub.chaincodeEvent, ChannelId: stub.ChannelId}
		log.Debugf("[%s]Init succeeded. Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_COMPLETED)
	}()
}
...
func (handler *Handler) beforeInit(e *fsm.Event) {
	...
	if msg.Type.String() == pb.PtnChaincodeMessage_INIT.String() {
...
func (handler *Handler) handleTransaction(msg *pb.PtnChaincodeMessage) {
	// The defer followed by triggering a go routine dance is needed to ensure that the previous state transition
	// is completed before the next one is triggered. The previous state transition is deemed complete only when
	// the beforeInit function is exited. Interesting bug fix!!
	go func() {
		...
		errFunc := func(err error, ce *pb.PtnChaincodeEvent, errStr string, args ...interface{}) *pb.PtnChaincodeMessage {
			if err != nil {
				payload := []byte(err.Error())
		...
		if nextStateMsg = errFunc(unmarshalErr, nil, "[%s]Incorrect payload format. Sending %s",
			shorttxid(msg.Txid), pb.PtnChaincodeMessage_ERROR.String()); nextStateMsg != nil {
			return
		}
		...
		// Send COMPLETED message to chaincode support and change state
		log.Debugf("[%s]Transaction completed. Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_COMPLETED)
		nextStateMsg = &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_COMPLETED, Payload: resBytes, Txid: msg.Txid,
			ChaincodeEvent: stub.chaincodeEvent, ChannelId: stub.ChannelId}
	}()
}
...
func (handler *Handler) beforeTransaction(e *fsm.Event) {
	...
	if msg.Type.String() == pb.PtnChaincodeMessage_TRANSACTION.String() {
...
func (handler *Handler) callPeerWithChaincodeMsg(msg *pb.PtnChaincodeMessage, channelID,
	txid string) (pb.PtnChaincodeMessage, error) {
	// Create the channel on which to communicate the response from the peer
	var respChan chan pb.PtnChaincodeMessage
	var err error
...
func (handler *Handler) handleGetState(collection string, key string, contractid []byte, channelId string,
	...
	payloadBytes, _ := proto.Marshal(&pb.PtnGetState{Collection: collection, ContractId: contractid, Key: key})

	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_PTN_GET_STATE, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractid}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_PTN_GET_STATE)
	...
	if responseMsg.Type.String() == pb.PtnChaincodeMessage_RESPONSE.String() {
		// Success response
		log.Debugf("[%s]GetState received payload %s", shorttxid(responseMsg.Txid),
			pb.PtnChaincodeMessage_RESPONSE)
		return responseMsg.Payload, nil
	}
	if responseMsg.Type.String() == pb.PtnChaincodeMessage_ERROR.String() {
		// Error response
		log.Errorf("[%s]GetState received error %s", shorttxid(responseMsg.Txid), pb.PtnChaincodeMessage_ERROR)
	...
	return nil, errors.Errorf("[%s]incorrect chaincode message %s received. Expecting %s or %s",
		shorttxid(responseMsg.Txid), responseMsg.Type, pb.PtnChaincodeMessage_RESPONSE, pb.PtnChaincodeMessage_ERROR)
}
...
func (handler *Handler) handelGetStateByPrefix(prefix string, contractId []byte, channelId string,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_GET_STATE_BY_PREFIX, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractId}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_GET_STATE_BY_PREFIX)
...
func (handler *Handler) handleGetTimestamp(collection string, rangeNumber uint32, contractid []byte,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_GET_TIMESTAMP, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractid}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_GET_TIMESTAMP)
...
func (handler *Handler) handleGetTokenBalance(address string, token *modules.Asset, contractid []byte,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_GET_TOKEN_BALANCE, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractid}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_GET_TOKEN_BALANCE)
...
func (handler *Handler) handleGetStableTransactionByHash(txHash string, contractid []byte,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_GET_STABLE_TRANSACTION, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractid}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_GET_STABLE_TRANSACTION)

	responseMsg, err := handler.callPeerWithChaincodeMsg(msg, channelId, txid)
	if err != nil {
		return nil, errors.WithMessage(err, fmt.Sprintf("[%s]error sending GET_STABLE_TRANSACTION", shorttxid(txid)))
	}
...
func (handler *Handler) handleGetStableUnit(unitHash string, unitNumber uint64, contractid []byte,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_GET_STABLE_UNIT, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractid}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_GET_STABLE_UNIT)
...
func (handler *Handler) handlePayOutToken(collection string, addr string, invokeTokens *modules.AmountAsset,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_PAY_OUT_TOKEN, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractid}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_PAY_OUT_TOKEN)
...
func (handler *Handler) handleDefineToken(tokenType byte, define []byte, creator string, contractid []byte,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_DEFINE_TOKEN, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractid}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_DEFINE_TOKEN)
	...
	if responseMsg.Type.String() == pb.PtnChaincodeMessage_ERROR.String() {
		// Error response
		log.Errorf("[%s]Received %s. Payload: %s", shorttxid(responseMsg.Txid), pb.PtnChaincodeMessage_ERROR,
			responseMsg.Payload)
		return errors.New(string(responseMsg.Payload[:]))
	}
...
func (handler *Handler) handleSupplyToken(assetId []byte, uniqueId []byte, amt uint64, creator string,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_SUPPLY_TOKEN, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractid}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_SUPPLY_TOKEN)
...
func (handler *Handler) handlePutState(collection string, contractId []byte, key string, value []byte,
	...
	payloadBytes, _ := proto.Marshal(&pb.PtnPutState{Collection: collection, ContractId: contractId, Key: key,
		Value: value})

	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_PTN_PUT_STATE, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractId}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_PTN_PUT_STATE)
...
func (handler *Handler) handleOutCall(collection string, outChainName string, method string, params []byte,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_OUTCHAIN_CALL, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_OUTCHAIN_CALL)
...
func (handler *Handler) handleSendJury(collection string, msgType uint32, consultContent []byte, myAnswer []byte,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_SEND_JURY, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_SEND_JURY)
	...
	if responseMsg.Type.String() == pb.PtnChaincodeMessage_ERROR.String() {
		...
		log.Errorf("[%s]Received %s. Payload: %s", shorttxid(responseMsg.Txid),
			pb.PtnChaincodeMessage_ERROR, responseMsg.Payload)
		return nil, errors.New(string(responseMsg.Payload[:]))
	}
...
func (handler *Handler) handleRecvJury(collection string, msgType uint32, consultContent []byte, timeout uint32,
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_RECV_JURY, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_RECV_JURY)
...
func (handler *Handler) handleDelState(collection string, contractId []byte, key string, channelId string,
	txid string) error {
	//payloadBytes, _ := proto.Marshal(&pb.PtnGetState{Collection: collection, Key: key})
	payloadBytes, _ := proto.Marshal(&pb.PtnDelState{Collection: collection, ContractId: contractId, Key: key})

	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_PTN_DEL_STATE, Payload: payloadBytes, Txid: txid,
		ChannelId: channelId, ContractId: contractId}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_PTN_DEL_STATE)

	// Execute the request and get response
	responseMsg, err := handler.callPeerWithChaincodeMsg(msg, channelId, txid)
	if err != nil {
		return errors.Errorf("[%s]error sending DEL_STATE %s", shorttxid(msg.Txid),
			pb.PtnChaincodeMessage_PTN_DEL_STATE)
	}
	...
	if responseMsg.Type.String() == pb.PtnChaincodeMessage_ERROR.String() {
		// Error response
		log.Errorf("[%s]Received %s. Payload: %s", msg.Txid, pb.PtnChaincodeMessage_ERROR, responseMsg.Payload)
		return errors.New(string(responseMsg.Payload[:]))
	}
...
func (handler *Handler) handleGetSystemConfig(channelId, txid string) (*modules.GlobalProperty, error) {
	...
	return nil, errors.Errorf("[%s]incorrect chaincode message %s received. Expecting %s",
		shorttxid(responseMsg.Txid), responseMsg.Type, pb.PtnChaincodeMessage_RESPONSE)
}
...
func (handler *Handler) handleGetContractAllState(channelId, txid string,
	...
	//定义一个pb.PtnChaincodeMessage_GET_ALL_SATE
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_GET_CONTRACT_ALL_STATE, Payload: []byte(""),
		ChannelId: channelId, Txid: txid, ContractId: contractid}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_GET_CONTRACT_ALL_STATE)
	//Execute the request and get response
	responseMsg, err := handler.callPeerWithChaincodeMsg(msg, channelId, txid)
	if err != nil {
	...
	if responseMsg.Type.String() == pb.PtnChaincodeMessage_RESPONSE.String() {
		...
		log.Debugf("[%s]Received %s. Successfully get tokens of pay to contract ",
			shorttxid(responseMsg.Txid), pb.PtnChaincodeMessage_RESPONSE)
...
func (handler *Handler) handleInvokeChaincode(chaincodeName string, args [][]byte, channelId string,
	...
	payloadBytes, _ := proto.Marshal(&pb.PtnChaincodeSpec{ChaincodeId: &pb.PtnChaincodeID{Name: chaincodeName},
	...
	msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_INVOKE_CHAINCODE, Payload: payloadBytes,
		Txid: txid, ChannelId: channelId}
	log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_INVOKE_CHAINCODE)

	var responseMsg pb.PtnChaincodeMessage
	...
	if responseMsg, err = handler.sendReceive(msg, respChan); err != nil {
		return handler.createResponse(ERROR, []byte(fmt.Sprintf("[%s]error sending %s",
			shorttxid(msg.Txid), pb.PtnChaincodeMessage_INVOKE_CHAINCODE)))
	}
	...
	if responseMsg.Type.String() == pb.PtnChaincodeMessage_RESPONSE.String() {
		...
		log.Debugf("[%s]Received %s. Successfully invoked chaincode", shorttxid(responseMsg.Txid),
			pb.PtnChaincodeMessage_RESPONSE)
		respMsg := &pb.PtnChaincodeMessage{}
		if err = proto.Unmarshal(responseMsg.Payload, respMsg); err != nil {
			return handler.createResponse(ERROR, []byte(err.Error()))
		}
		if respMsg.Type == pb.PtnChaincodeMessage_COMPLETED {
			// Success response
			log.Debugf("[%s]Received %s. Successfully invoked chaincode", shorttxid(responseMsg.Txid),
				pb.PtnChaincodeMessage_RESPONSE)
			res := &pb.Response{}
	...
	if responseMsg.Type.String() == pb.PtnChaincodeMessage_ERROR.String() {
		// Error response
		log.Errorf("[%s]Received %s.", shorttxid(responseMsg.Txid), pb.PtnChaincodeMessage_ERROR)
		return handler.createResponse(ERROR, responseMsg.Payload)
	}

	// Incorrect chaincode message received
	return handler.createResponse(ERROR, []byte(fmt.Sprintf("[%s]Incorrect chaincode message %s received."+
		" Expecting %s or %s", shorttxid(responseMsg.Txid), responseMsg.Type, pb.PtnChaincodeMessage_RESPONSE,
		pb.PtnChaincodeMessage_ERROR)))
}
...
func (handler *Handler) handleMessage(msg *pb.PtnChaincodeMessage) error {
	if msg.Type == pb.PtnChaincodeMessage_KEEPALIVE {
	...
	if handler.FSM.Cannot(msg.Type.String()) {
		...
		handler.serialSend(&pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_ERROR, Payload: []byte(err.Error()),
...
func newChaincodeHandler(peerChatStream PeerChaincodeStream, chaincode Chaincode) *Handler {
	...
	v.responseChannel = make(map[string]chan pb.PtnChaincodeMessage)
	...
	v.FSM = fsm.NewFSM(
		"created",
		fsm.Events{
			{Name: pb.PtnChaincodeMessage_REGISTERED.String(), Src: []string{"created"}, Dst: "established"},
			{Name: pb.PtnChaincodeMessage_READY.String(), Src: []string{"established"}, Dst: "ready"},
			{Name: pb.PtnChaincodeMessage_ERROR.String(), Src: []string{"init"}, Dst: "established"},
			{Name: pb.PtnChaincodeMessage_RESPONSE.String(), Src: []string{"init"}, Dst: "init"},
			{Name: pb.PtnChaincodeMessage_INIT.String(), Src: []string{"ready"}, Dst: "ready"},
			{Name: pb.PtnChaincodeMessage_TRANSACTION.String(), Src: []string{"ready"}, Dst: "ready"},
			{Name: pb.PtnChaincodeMessage_RESPONSE.String(), Src: []string{"ready"}, Dst: "ready"},
			{Name: pb.PtnChaincodeMessage_ERROR.String(), Src: []string{"ready"}, Dst: "ready"},
			{Name: pb.PtnChaincodeMessage_COMPLETED.String(), Src: []string{"init"}, Dst: "ready"},
			{Name: pb.PtnChaincodeMessage_COMPLETED.String(), Src: []string{"ready"}, Dst: "ready"},
		},
		fsm.Callbacks{
			"before_" + pb.PtnChaincodeMessage_REGISTERED.String():  func(e *fsm.Event) { v.beforeRegistered(e) },
			"after_" + pb.PtnChaincodeMessage_RESPONSE.String():     func(e *fsm.Event) { v.afterResponse(e) },
			"after_" + pb.PtnChaincodeMessage_ERROR.String():        func(e *fsm.Event) { v.afterError(e) },
			"before_" + pb.PtnChaincodeMessage_INIT.String():        func(e *fsm.Event) { v.beforeInit(e) },
			"before_" + pb.PtnChaincodeMessage_TRANSACTION.String(): func(e *fsm.Event) { v.beforeTransaction(e) },
		},
	)
	return v
}
...
//
// msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_GET_QUERY_RESULT, Payload: payloadBytes, Txid: txid,
// ChannelId: channelId}
// log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_GET_QUERY_RESULT)
//
// responseMsg, err := handler.callPeerWithChaincodeMsg(msg, channelId, txid)
// if err != nil {
//    return nil, errors.Errorf("[%s]error sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_GET_QUERY_RESULT)
// }
//
// if responseMsg.Type.String() == pb.PtnChaincodeMessage_RESPONSE.String() {
//    // Success response
//    log.Debugf("[%s]Received %s. Successfully got range", shorttxid(responseMsg.Txid), pb.PtnChaincodeMessage_RESPONSE)
//
...
// pb.PtnChaincodeMessage_RESPONSE, pb.PtnChaincodeMessage_ERROR)
//}
/*
func (handler *Handler) handleGetHistoryForKey(key string, channelId string, txid string) (*pb.QueryResponse, error) {
   ...
   msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_GET_HISTORY_FOR_KEY, Payload: payloadBytes,
Txid: txid, ChannelId: channelId}
   log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_GET_HISTORY_FOR_KEY)

   var responseMsg pb.PtnChaincodeMessage

   if responseMsg, err = handler.sendReceive(msg, respChan); err != nil {
      return nil, errors.Errorf("[%s]error sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_GET_HISTORY_FOR_KEY)
   }
...
txid string) (*pb.QueryResponse, error) {
   ...
   //we constructed a valid object. No need to check for error
   payloadBytes, _ := proto.Marshal(&pb.PtnGetStateByRange{Collection: collection, StartKey: startKey, EndKey: endKey})

   msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_PTN_GET_STATE_BY_RANGE, Payload: payloadBytes,
Txid: txid, ChannelId: channelId}
   log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_PTN_GET_STATE_BY_RANGE)

   responseMsg, err := handler.callPeerWithChaincodeMsg(msg, channelId, txid)
   if err != nil {
      return nil, errors.Errorf("[%s]error sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_PTN_GET_STATE_BY_RANGE)
   }
...
func (handler *Handler) handleQueryStateNext(id, channelId, txid string) (*pb.QueryResponse, error) {
   ...
   msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_QUERY_STATE_NEXT, Payload: payloadBytes,
Txid: txid, ChannelId: channelId}
   log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_QUERY_STATE_NEXT)

   var responseMsg pb.PtnChaincodeMessage

   if responseMsg, err = handler.sendReceive(msg, respChan); err != nil {
      return nil, errors.Errorf("[%s]error sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_QUERY_STATE_NEXT)
   }
...
func (handler *Handler) handleQueryStateClose(id, channelId, txid string) (*pb.QueryResponse, error) {
   ...
   msg := &pb.PtnChaincodeMessage{Type: pb.PtnChaincodeMessage_QUERY_STATE_CLOSE, Payload: payloadBytes,
Txid: txid, ChannelId: channelId}
   log.Debugf("[%s]Sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_QUERY_STATE_CLOSE)

   var responseMsg pb.PtnChaincodeMessage

   if responseMsg, err = handler.sendReceive(msg, respChan); err != nil {
      return nil, errors.Errorf("[%s]error sending %s", shorttxid(msg.Txid), pb.PtnChaincodeMessage_QUERY_STATE_CLOSE)
   }
