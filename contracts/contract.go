...
type ContractInf interface {
	...
	Invoke(chainID string, deployId []byte, txid string, tx *unit.Transaction, args [][]byte, timeout time.Duration) (*modules.ContractInvokeResult, error, dag.IDag)
...
func (c *Contract) Invoke(chainID string, deployId []byte, txid string, tx *unit.Transaction, args [][]byte, timeout time.Duration) (*modules.ContractInvokeResult, error, dag.IDag) {
	atomic.LoadInt32(&initFlag)
	if initFlag == 0 {
		return nil, errors.New("Contract not initialized"), nil
	}
	contractInvokeResult, err := cc.Invoke(deployId, c.dag, chainID, deployId, txid, tx, args, timeout)
	return contractInvokeResult, err, c.dag
}
