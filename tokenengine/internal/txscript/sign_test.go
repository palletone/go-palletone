...
func mkGetKey(keys map[string]addressToKey) txscript.KeyDB {
	if keys == nil {
		return txscript.KeyClosure(func(addr common.Address) (*btcec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) txscript.ScriptDB {
	if scripts == nil {
		return txscript.ScriptClosure(func(addr common.Address) (
			[]byte, error) {
			return nil, errors.New("nope")
		})
	}
	return txscript.ScriptClosure(func(addr common.Address) ([]byte,
		error) {
...
func checkScripts(msg string, tx *modules.Transaction, idx int, sigScript, pkScript []byte) error {
...
func signAndCheck(msg string, tx *modules.Transaction, idx int, pkScript []byte,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pkScript, err := txscript.MultiSigScript(
				[]*common.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
