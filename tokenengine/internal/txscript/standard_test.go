...
// newAddressPubKey returns a new common.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) common.Address {
...
// newAddressPubKeyHash returns a new common.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) common.Address {
...
// newAddressScriptHash returns a new common.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) common.Address {
...
// bogusAddress implements the common.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the common.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	tests := []struct {
		...
		{(*common.AddressPubKeyHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*common.AddressScriptHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*common.AddressPubKey)(nil), "", txscript.ErrUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*common.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*common.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
