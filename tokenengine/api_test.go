...
func TestGetAddressFromScript(t *testing.T) {
	...
	p2pkhLock := Instalnce.GenerateP2PKHLockScript(addr.Bytes())
	t.Logf("P2PKH script:%x", p2pkhLock)
	getAddr, _ := Instalnce.GetAddressFromScript(p2pkhLock)
	...
	p2shLock := Instalnce.GenerateP2SHLockScript(addr2.Bytes())
	getAddr2, _ := Instalnce.GetAddressFromScript(p2shLock)
...
func TestGenerateP2CHLockScript(t *testing.T) {
	...
	assert.Nil(t, err)
	p2ch1 := Instalnce.GenerateLockScript(addr)
	p2ch1Str, _ := Instalnce.DisasmString(p2ch1)
	t.Logf("Pay to contract hash lock script:%x, string:%s", p2ch1, p2ch1Str)
	p2ch2 := Instalnce.GenerateP2CHLockScript(addr)
	assert.Equal(t, p2ch1, p2ch2)
	addr2, err := Instalnce.GetAddressFromScript(p2ch1)
	...
	script := Instalnce.GenerateP2CHLockScript(deposit)
	t.Logf("%x", script)
}
...
func TestSignAndVerify2PaymentTx(t *testing.T) {
	...
	lockScript := Instalnce.GenerateP2PKHLockScript(pubKeyHash)
	...
	lockScripts := map[modules.OutPoint][]byte{
		*outPoint:  lockScript[:],
		*outPoint2: Instalnce.GenerateP2PKHLockScript(pubKeyHash),
	}
	...
	var hashtype uint32
	hashtype = 1
	_, err := Instalnce.SignTxAllPaymentInput(tx, hashtype, lockScripts, nil, getPubKeyFn, getSignFn)
	if err != nil {
	...
	s, _ := Instalnce.DisasmString(unlockScript)
	t.Logf("UnlockScript string:%s", s)
	err = Instalnce.ScriptValidate(lockScript, nil, tx, 0, 0)
	if err != nil {
	...
	err = Instalnce.ScriptValidate(lockScript, nil, tx, 1, 0)
...
func build23Address() ([]byte, []byte, string) {
	...
	redeemScript := Instalnce.GenerateRedeemScript(2, [][]byte{pubKey1B, pubKey2B, pubKey3B})
	lockScript := Instalnce.GenerateP2SHLockScript(crypto.Hash160(redeemScript))
	addressMulti, _ := Instalnce.GetAddressFromScript(lockScript)

	return lockScript, redeemScript, addressMulti.Str()
}
func build22Address() ([]byte, []byte, string) {

	redeemScript := Instalnce.GenerateRedeemScript(2, [][]byte{pubKey1B, pubKey2B})
	lockScript := Instalnce.GenerateP2SHLockScript(crypto.Hash160(redeemScript))
	addressMulti, _ := Instalnce.GetAddressFromScript(lockScript)
...
func TestMultiSign1Step(t *testing.T) {
	...
	r, _ := Instalnce.DisasmString(redeemScript)
	...
	txIn := modules.NewTxIn(outPoint, []byte{})
	payment.AddTxIn(txIn)
	p1lockScript := Instalnce.GenerateP2PKHLockScript(crypto.Hash160(pubKey1B))
	...
	sign12, err := Instalnce.MultiSignOnePaymentInput(tx, SigHashAll, 0, 0, lockScript, redeemScript, getPubKeyFn, getSignFn, nil)
	if err != nil {
		t.Logf("Sign error:%s", err)
	}
...
func TestMultiSign2Step(t *testing.T) {
	...
	sign1, err := Instalnce.MultiSignOnePaymentInput(tx, SigHashAll, 0, 0, lockScript, redeemScript, getPubKeyFn, getSignFn, nil)
	if err != nil {
		t.Logf("Sign error:%s", err)
	}
	...
	sign2, err := Instalnce.MultiSignOnePaymentInput(tx, SigHashAll, 0, 0, lockScript, redeemScript, getPubKeyFn, getSignFn, sign1)
	if err != nil {
		t.Logf("Sign error:%s", err)
	}
...
func mockPickupJuryRedeemScript(addr common.Address) ([]byte, error) {
	return Instalnce.GenerateRedeemScript(2, [][]byte{pubKey1B, pubKey2B, pubKey3B, pubKey4B}), nil
}
func TestContractPayout(t *testing.T) {
	...
	lockScript := Instalnce.GenerateP2CHLockScript(contractAddr) //Token 锁定到保证金合约中
	...
	//Sign input0
	sign0, err := Instalnce.MultiSignOnePaymentInput(tx, SigHashRaw, 0, 0, lockScript, redeemScript, getPubKeyFn, getSignFn, nil)
	if err != nil {
		t.Logf("Sign error:%s", err)
	}
	t.Logf("PrvKey1&2 sign result:%x\n", sign0)
	...
	t.Logf("Signed script:{%s}", str)
	//Sign input1
	sign1, err := Instalnce.MultiSignOnePaymentInput(tx, SigHashRaw, 0, 1, lockScript, redeemScript, getPubKeyFn, getSignFn, nil)
	if err != nil {
		t.Logf("Sign error:%s", err)
	}
	t.Logf("PrvKey1&2 sign result:%x\n", sign1)
	...
	err = Instalnce.ScriptValidate(lockScript, mockPickupJuryRedeemScript, tx, 0, 0)
	assert.Nil(t, err, fmt.Sprintf("validate error:%s", err))

	err = Instalnce.ScriptValidate(lockScript, mockPickupJuryRedeemScript, tx, 0, 1)
	assert.Nil(t, err, fmt.Sprintf("validate error:%s", err))
...
func TestGenerateRedeemScript(t *testing.T) {
	...
	redeem := Instalnce.GenerateRedeemScript(2, pubKeys)
...
func TestUserContractPayout(t *testing.T) {
	...
	//copy(scriptCp,lockScript)
	contractAddr, _ := common.StringToAddress("PCGTta3M4t3yXu8uRgkKvaWd2d8DR32W9vM")
	lockScript := Instalnce.GenerateP2CHLockScript(contractAddr) //Token 锁定到合约中
	l, _ := txscript.DisasmString(lockScript)
	t.Logf("Lock Script:%s", l)
	getPubKeyFn := func(addr common.Address) ([]byte, error) {
		if addr == address1 {
	...
	//签名所有input中锁定脚本为空的情况。
	errMsg, err := Instalnce.SignTxAllPaymentInput(tx, SigHashRaw, lockScripts, redeemScript, getPubKeyFn, getSignFn)
	if err != nil {
		t.Logf("Sign error:%s", err)
	}
	if len(errMsg) > 0 {
	...
	addrs, err := Instalnce.GetScriptSigners(tx, 0, 0)
	assert.Nil(t, err)
...
func TestMergeContractUnlockScript(t *testing.T) {
	...
	result := Instalnce.MergeContractUnlockScript([][]byte{sign1, sign2, sign3}, redeemScript)
	t.Logf("%x", result)
	txt, _ := Instalnce.DisasmString(result)
	t.Log(txt)
}
...
func generateSignedTx() (*modules.Transaction, []byte) {
	...
	var hashtype uint32
	hashtype = 1
	Instalnce.SignTxAllPaymentInput(tx, hashtype, lockScripts, nil, getPubKeyFn, getSignFn)
