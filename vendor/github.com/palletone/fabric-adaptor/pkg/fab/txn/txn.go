...
func New(request fab.TransactionRequest) (*fab.Transaction, error) {
	...
	cea := &pb.ChaincodeEndorsedAction{ProposalResponsePayload: responsePayload,
...
// Send send a transaction to the chain’s orderer service (one or more orderer endpoints)
// for consensus and committing to the ledger.
func Send(reqCtx reqContext.Context, tx *fab.Transaction,
	orderers []fab.Orderer) (*fab.TransactionResponse, error) {
	if len(orderers) == 0 {
...
func SignZxl(reqCtx reqContext.Context, tx *fab.Transaction,
	orderers []fab.Orderer) (*fab.SignedEnvelope, error) {//Zxl add
	if len(orderers) == 0 {
		return nil, errors.New("orderers is nil")
	}
	if tx == nil {
	...
	signedEnvelope, err := SignPayloadZxl(reqCtx, &payload, orderers)
	if err != nil {
		return nil, err
	}
...
func SendZxl(reqCtx reqContext.Context, envelope *fab.SignedEnvelope,
	orderers []fab.Orderer) (*fab.TransactionResponse, error) {//Zxl add
	if len(orderers) == 0 {
		return nil, errors.New("orderers is nil")
	}

	transactionResponse, err := BroadcastPayloadZxl(reqCtx, envelope, orderers)
	if err != nil {
		return nil, err
	}
...
func BroadcastPayload(reqCtx reqContext.Context, payload *common.Payload,
	orderers []fab.Orderer) (*fab.TransactionResponse, error) {
...
func SignPayloadZxl(reqCtx reqContext.Context, payload *common.Payload,
	orderers []fab.Orderer) (*fab.SignedEnvelope, error) {//Zxl add
	ctx, ok := context.RequestClientContext(reqCtx)
	if !ok {
		return nil, errors.New("failed get client context from reqContext for signPayload")
...
func BroadcastPayloadZxl(reqCtx reqContext.Context, envelope *fab.SignedEnvelope,
	orderers []fab.Orderer) (*fab.TransactionResponse, error) {//Zxl add
	// Check if orderers are defined
	if len(orderers) == 0 {
		return nil, errors.New("orderers not set")
...
// until all are exhausted
func broadcastEnvelope(reqCtx reqContext.Context, envelope *fab.SignedEnvelope,
	orderers []fab.Orderer) (*fab.TransactionResponse, error) {
	// Check if orderers are defined
	if len(orderers) == 0 {
		return nil, errors.New("orderers not set")
	...
	// get a context client instance to create child contexts with timeout read
	// from the config in sendBroadcast()
...
func sendBroadcast(reqCtx reqContext.Context, envelope *fab.SignedEnvelope,
	orderer fab.Orderer, client ctxprovider.Client) (*fab.TransactionResponse, error) {
	...
	childCtx, cancel := context.NewRequest(client, context.WithTimeoutType(fab.OrdererResponse),
		context.WithParent(reqCtx))
	defer cancel()
...
func SendPayload(reqCtx reqContext.Context, payload *common.Payload,
	orderers []fab.Orderer) (*common.Block, error) {
	if len(orderers) == 0 {
		return nil, errors.New("orderers not set")
	}
...
func sendEnvelope(reqCtx reqContext.Context, envelope *fab.SignedEnvelope,
	orderer fab.Orderer) (*common.Block, error) {
