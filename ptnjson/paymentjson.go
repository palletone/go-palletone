	...
	for _, input := range paymentJson.Inputs {
		if input.TxHash != "" {
			...
			if err != nil {
				...
				addr, _ := tokenengine.Instalnce.GetAddressFromScript(utxo.PkScript)
...
func ConvertPayment2Json(payment *modules.PaymentPayload) *PaymentJson {
	...
	if len(payment.Inputs) > 0 {
		for _, in := range payment.Inputs {
			...
			if in.SignatureScript != nil {
				unlock, _ = tokenengine.Instalnce.DisasmString(in.SignatureScript)
			}
	...
	for _, out := range payment.Outputs {
		addr, _ := tokenengine.Instalnce.GetAddressFromScript(out.PkScript)
		lock, _ := tokenengine.Instalnce.DisasmString(out.PkScript)
