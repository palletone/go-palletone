...
func createPayment(fromAddr, toAddr common.Address, amountToken uint64, feePTN uint64,
	...
	//ptn outputs
	if amountToken > 0 {
		payPTN.AddTxOut(modules.NewTxOut(amountToken, tokenengine.Instalnce.GenerateLockScript(toAddr), asset))
	}
	if change > 0 {
		payPTN.AddTxOut(modules.NewTxOut(change, tokenengine.Instalnce.GenerateLockScript(fromAddr), asset))
	}
	//
	////Token
...
func WalletCreateTransaction(c *ptnjson.CreateRawTransactionCmd) (string, error) {
	...
	for _, addramt := range c.Amounts {
		...
		pkScript := tokenengine.Instalnce.GenerateLockScript(addr)
		ppscript = pkScript
	...
	for msgindex, msg := range mtxtmp.TxMessages {
		...
		for inputindex := range payload.Inputs {
			hashforsign, err := tokenengine.Instalnce.CalcSignatureHash(mtxtmp, tokenengine.SigHashAll, msgindex, inputindex, ppscript)
			if err != nil {
				return "", err
			}
...
func (s *PrivateWalletAPI) SignRawTransaction(ctx context.Context, params string, hashtype string, password string, duration *uint64) (ptnjson.SignRawTransactionResult, error) {
	...
	for _, msg := range tx.TxMessages {
		...
		for _, txin := range payload.Inputs {
			...
			addr, err = tokenengine.Instalnce.GetAddressFromScript(hexutil.MustDecode(uvu.PkScriptHex))
			if err != nil {
				log.Error(err.Error())
...
func (s *PublicWalletAPI) CreateProofTransaction(ctx context.Context, params string, password string) (common.Hash, error) {
	...
	PkScript := tokenengine.Instalnce.GenerateLockScript(from)
	...
	for _, msg := range tx.TxMessages {
		...
		for _, txin := range payload.Inputs {
			...
			srawinputs = append(srawinputs, input)
			addr, err = tokenengine.Instalnce.GetAddressFromScript(hexutil.MustDecode(PkScriptHex))
			if err != nil {
				return common.Hash{}, err
			}
		}
...
func (s *PrivateWalletAPI) TransferToken(ctx context.Context, asset string, from string, to string,
	...
	//3.
	_, err = tokenengine.Instalnce.SignTxAllPaymentInput(rawTx, 1, utxoLockScripts, nil, getPubKeyFn, getSignFn)
	if err != nil {
		return common.Hash{}, err
	}
