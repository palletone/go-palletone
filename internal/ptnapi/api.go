...
func SignRawTransaction(cmd *ptnjson.SignRawTransactionCmd, pubKeyFn tokenengine.AddressGetPubKey, hashFn tokenengine.AddressGetSign, addr common.Address) (ptnjson.SignRawTransactionResult, error) {
	...
	signErrs, err = tokenengine.Instalnce.SignTxAllPaymentInput(tx, hashType, inputpoints, redeem, pubKeyFn, hashFn)
	if err != nil {
		return ptnjson.SignRawTransactionResult{}, DeserializationError{err}
	}
	for msgidx, msg := range tx.TxMessages {
		...
		for inputindex := range payload.Inputs {
			err = tokenengine.Instalnce.ScriptValidate(PkScript, nil, tx, msgidx, inputindex)
			if err != nil {
				return ptnjson.SignRawTransactionResult{}, DeserializationError{err}
			}
		}
		...
		for k := range payload.Outputs {
			...
			case tokenengine.SigHashSingle:
				...
				pk_addr, err := tokenengine.Instalnce.GetAddressFromScript(payload.Outputs[k].PkScript)
				if err != nil {
...
func (s *PublicTransactionPoolAPI) BatchSign(ctx context.Context, txid string, fromAddress, toAddress string, amount int, count int, password string) ([]string, error) {
	...
	utxoScript := tokenengine.Instalnce.GenerateLockScript(fromAddr)
	ks := s.b.GetKeyStore()
	...
	for i := 0; i < count; i++ {
		...
		lockScript := tokenengine.Instalnce.GenerateLockScript(toAddr)
		...
		errs, err := tokenengine.Instalnce.SignTxAllPaymentInput(tx, tokenengine.SigHashAll, utxoLookup, nil, func(addresses common.Address) ([]byte, error) {
			return pubKey, nil
		},
