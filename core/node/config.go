...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gpan is "gpan". If no
...
// These resources are resolved differently for "gpan" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	// by gpan 1.4 are used if they exist.
	if c.name() == "gpan" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "gpan" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-palletone-keystore")
		ephemeral = keydir
	}
