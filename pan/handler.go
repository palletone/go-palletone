...
func NewProtocolManager(config *configure.ChainConfig, mode downloader.SyncMode, networkId uint64, mux *event.TypeMux, txpool txPool, engine core.ConsensusEngine, blockchain *coredata.BlockChain, chaindb pandb.Database) (*ProtocolManager, error) {
	...
	for i, version := range ProtocolVersions {
		...
		if mode == downloader.FastSync && version < pan3 {
			continue
		}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= pan3 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= pan3 && msg.Code == NodeDataMsg:
	...
	case p.version >= pan3 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= pan3 && msg.Code == ReceiptsMsg:
