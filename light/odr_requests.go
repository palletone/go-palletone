...
type LesOdrRequest interface {
	...
	Validate(ptndb.Database, *Msg) error
}

func LesRequest(req les.OdrRequest) LesOdrRequest {
	switch r := req.(type) {
	case *les.BlockRequest:
		return (*BlockRequest)(r)
	case *les.TrieRequest:
		return (*TrieRequest)(r)
	case *les.CodeRequest:
...
type BlockRequest les.BlockRequest
...
func (r *BlockRequest) Request(reqID uint64, peer *peer) error {
	log.Debug("Requesting block body", "hash", r.Hash)
	return nil
...
func (r *BlockRequest) Validate(db ptndb.Database, msg *Msg) error {
	...
	bodies := msg.Obj.([]*modules.Transaction)
	if len(bodies) != 1 {
	...
	//if header.TxHash != types.DeriveSha(modules.Transactions(body.Transactions)) {
...
//type ReceiptsRequest les.ReceiptsRequest
//
//// GetCost returns the cost of the given ODR request according to the serving
//// peer's cost table (implementation of LesOdrRequest)
...
//	log.Debug("Requesting block receipts", "hash", r.Hash)
//	return nil
...
type TrieRequest les.TrieRequest
...
func (r *TrieRequest) GetCost(peer *peer) uint64 {
	switch peer.version {
	case lpv1:
		return uint64(0) //peer.GetRequestCost(GetProofsV1Msg, 1)
	case lpv2:
		return uint64(0) //peer.GetRequestCost(GetProofsV2Msg, 1)
	default:
		panic(nil)
	}
}
...
func (r *TrieRequest) Request(reqID uint64, peer *peer) error {
	log.Debug("Requesting trie proof", "root", r.Id.Root, "key", r.Key)
	req := ProofReq{
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LesOdrRequest)
func (r *TrieRequest) Validate(db ptndb.Database, msg *Msg) error {
	log.Debug("Validating trie proof", "root", r.Id.Root, "key", r.Key)

	switch msg.MsgType {
	case MsgProofsV1:
		proofs := msg.Obj.([]les.NodeList)
		if len(proofs) != 1 {
			return errInvalidEntryCount
		}
...
type CodeRequest les.CodeRequest
...
func (r *CodeRequest) Request(reqID uint64, peer *peer) error {
	log.Debug("Requesting code data", "hash", r.Hash)
	return nil
	//req := CodeReq{
	//	BHash:  r.Id.BlockHash,
	//	AccKey: r.Id.AccKey,
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LesOdrRequest)
func (r *CodeRequest) Validate(db ptndb.Database, msg *Msg) error {
...
type ChtRequest les.ChtRequest
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LesOdrRequest)
func (r *ChtRequest) Validate(db ptndb.Database, msg *Msg) error {
	...
	case MsgHeaderProofs: // LES/1 backwards compatibility
		...
		value, err, _ := trie.VerifyProof(r.ChtRoot, encNumber[:], les.NodeList(proof.Proof).NodeSet())
		if err != nil {
			return err
		}
		var node les.ChtNode
		...
		r.Proof = les.NodeList(proof.Proof).NodeSet()
		r.Td = node.Td
	case MsgHelperTrieProofs:
		...
		if r.BlockNum != header.Number.Index {
