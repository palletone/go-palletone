...
type TxPool struct {
	...
	pending  map[common.Hash]*modules.Transaction   // pending transactions by tx hash
	mined    map[common.Hash][]*modules.Transaction // mined transactions by block hash
...
func NewTxPool( /*config *params.ChainConfig, chain *LightChain,*/ relay TxRelayBackend) *TxPool {
	pool := &TxPool{
		...
		mined:       make(map[common.Hash][]*modules.Transaction),
		quit:        make(chan bool),
		...
		//clearIdx:    chain.CurrentHeader().Number.Index,
	}
...
/*
func (pool *TxPool) reorgOnNewHead(ctx context.Context, newHeader *modules.Header) (txStateChanges, error) {
...
func (pool *TxPool) setNewHead(head *modules.Header) {
	//pool.mu.Lock()
...
func (pool *TxPool) validateTx(ctx context.Context, tx *modules.Transaction) error {
	// Validate sender
	return nil
	//var (
	//	from common.Address
...
func (self *TxPool) add(ctx context.Context, tx *modules.Transaction) error {
	/*
		hash := tx.Hash()
...
func (self *TxPool) Add(ctx context.Context, tx *modules.Transaction) error {
	...
	self.relay.Send(modules.Transactions{tx})
...
func (self *TxPool) AddBatch(ctx context.Context, txs []*modules.Transaction) {
	self.mu.Lock()
	defer self.mu.Unlock()
	var sendTx modules.Transactions
...
// and nil otherwise.
func (tp *TxPool) GetTransaction(hash common.Hash) *modules.Transaction {
	// check the txs first
...
func (self *TxPool) GetTransactions() (txs modules.Transactions, err error) {
	self.mu.RLock()
	defer self.mu.RUnlock()

	txs = make(modules.Transactions, len(self.pending))
	i := 0
...
func (self *TxPool) Content() (map[common.Address]modules.Transactions, map[common.Address]modules.Transactions) {
	self.mu.RLock()
	defer self.mu.RUnlock()
...
func (self *TxPool) RemoveTransactions(txs modules.Transactions) {
	self.mu.Lock()
	defer self.mu.Unlock()
