...
func (c *Console) init(preload []string) error {
	...
	flatten := "var ptn = web3.ptn; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ptn.getBalance(ptn.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gptn instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gptn metadata
	fmt.Fprintf(c.printer, "Welcome to the Gptn JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("at unit: " + ptn.blockNumber + " (" + new Date(1000 * ptn.getBlock(ptn.blockNumber).timestamp) + ")");
