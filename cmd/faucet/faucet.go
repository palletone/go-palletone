...
type faucet struct {
	config *configure.ChainConfig // Chain configurations for signing
...
func newFaucet(genesis *coredata.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/palletone/go-palletone/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
