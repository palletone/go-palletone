...
import (
	...
	"github.com/palletone/go-palletone/whisper/mailserver"
...
func sendMsg(payload []byte) common.Hash {
	configure := whisper.MessageParams{
		Src:      asymKey,
		Dst:      pub,
		KeySym:   symKey,
		Payload:  payload,
	...
	msg, err := whisper.NewSentMessage(&configure)
	if err != nil {
		utils.Fatalf("failed to create new message: %s", err)
	}

	envelope, err := msg.Wrap(&configure)
	if err != nil {
...
func requestExpiredMessagesLoop() {
	...
	for {
		...
		var configure whisper.MessageParams
		configure.PoW = *argServerPoW
		configure.Payload = data
		configure.KeySym = key
		configure.Src = asymKey
		configure.WorkTime = 5

		msg, err := whisper.NewSentMessage(&configure)
		if err != nil {
			utils.Fatalf("failed to create new message: %s", err)
		}
		env, err := msg.Wrap(&configure)
		if err != nil {
