...
func newTx1(t *testing.T) *modules.Transaction {
	...
	_, err := tokenengine.Instalnce.SignTxAllPaymentInput(tx, 1, lockScripts, nil, getPubKeyFn, getSignFn)
	if err != nil {
...
func TestSignAndVerifyATx(t *testing.T) {
	...
	lockScript := tokenengine.Instalnce.GenerateP2PKHLockScript(pubKeyHash)
	...
	lockScripts := map[modules.OutPoint][]byte{
		*outPoint:  lockScript[:],
		*outPoint2: tokenengine.Instalnce.GenerateP2PKHLockScript(pubKeyHash),
	}
	...
	var hashtype uint32
	hashtype = 1
	_, err := tokenengine.Instalnce.SignTxAllPaymentInput(tx, hashtype, lockScripts, nil, getPubKeyFn, getSignFn)
	if err != nil {
		t.Logf("Sign error:%s", err)
	}
	unlockScript := tx.TxMessages[0].Payload.(*modules.PaymentPayload).Inputs[0].SignatureScript
