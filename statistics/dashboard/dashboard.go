...
func (db *Dashboard) Start(server *p2p.Server, corss *p2p.Server) error {
...
func (db *Dashboard) apiHandler(conn *websocket.Conn) {
	...
	versionMeta := ""
	if len(configure.VersionMeta) > 0 {
		versionMeta = fmt.Sprintf(" (%s)", configure.VersionMeta)
	}
	// Send the past data.
	client.msg <- Message{
		General: &GeneralMessage{
			Version: fmt.Sprintf("v%d.%d.%d%s", configure.VersionMajor, configure.VersionMinor, configure.VersionPatch, versionMeta),
			Commit:  db.commit,
		},
...
func (db *Dashboard) collectData() {
	...
	var (
		...
		prevDiskRead       = metrics.DefaultRegistry.Get("ptn/db/chaindata/disk/read").(metrics.Meter).Count()
		prevDiskWrite      = metrics.DefaultRegistry.Get("ptn/db/chaindata/disk/write").(metrics.Meter).Count()
	...
	for {
		...
		case <-time.After(db.config.Refresh):
			...
			var (
				...
				curDiskRead       = metrics.DefaultRegistry.Get("ptn/db/chaindata/disk/read").(metrics.Meter).Count()
				curDiskWrite      = metrics.DefaultRegistry.Get("ptn/db/chaindata/disk/write").(metrics.Meter).Count()
