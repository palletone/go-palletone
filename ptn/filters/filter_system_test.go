...
func (b *testBackend) ChainDb() ptndb.Database {
	return b.db
}
...
func (b *testBackend) BloomStatus() (uint64, uint64) {
	return configure.BloomBitsBlocks, b.sections
}
			...
			case request := <-requests:
				...
				for i, section := range task.Sections {
					if rand.Int()%4 != 0 { // Handle occasional missing deliveries
						head := core.GetCanonicalHash(b.db, (section+1)*configure.BloomBitsBlocks-1)
...
func TestBlockSubscription(t *testing.T) {
	...
	var (
		...
		chain, _    = core.GenerateChain(configure.TestChainConfig, genesis, ethash.NewFaker(), db, 10, func(i int, gen *core.BlockGen) {})
