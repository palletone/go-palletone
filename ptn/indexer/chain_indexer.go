...
type ChainIndexer struct {
	chainDb  ptndb.Database      // Chain database to index the data from
	indexDb  ptndb.Database      // Prefixed table-view of the db to write index metadata into
...
func (c *ChainIndexer) Start(chain ChainIndexerChain, assetId modules.AssetId) {
	events := make(chan modules.ChainEvent, 10)
	sub := chain.SubscribeChainEvent(events)

	go c.eventLoop(chain.CurrentHeader(assetId), events, sub)
}
		...
		case <-c.update:
			...
			if c.knownSections > c.storedSections {
				...
				if err == nil && oldHead == c.SectionHead(section-1) {
					...
					for _, child := range c.children {
						log.Trace("Cascading chain index update", "head", c.cascadedHead)
