...
type LightDag interface {
	...
	InsertHeaderChain([]*modules.Header, int) (int, error)
...
func New(mode SyncMode, mux *event.TypeMux, dropPeer peerDropFn, dag BlockDag, levelDb *palletdb.LDBDatabase) *Downloader {
	dl := &Downloader{
		mode:          mode,
		levelDb:       levelDb,
		mux:           mux,
...
func (d *Downloader) synchronise(id string, hash common.Hash, index uint64, mode SyncMode) error {
	...
	log.Info("Downloader->synchronise", "d.synchronising:", d.synchronising)
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, index uint64) (err error) {
	...
	pivot := uint64(0)
	log.Info("Downloader->syncWithPeer pre", "height:", height, "origin:", origin, "pivot:", pivot)
	if d.mode == FastSync {
	...
	if d.mode == FastSync && pivot != 0 {
		log.Info("======d.committed = 0")
		d.committed = 0
	}
	// Initiate the sync using a concurrent header and content retrieval algorithm
	d.queue.Prepare(origin+1, d.mode)
	if d.syncInitHook != nil {
		log.Info("syncInitHook!=nil")
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if len(headers) != 1 {
				...
				log.Info("Multiple headers for single request", "headers", len(headers))
		...
		case <-timeout:
			...
			log.Info("Waiting for head header timed out", "elapsed", ttl)
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if packet.Items() == 0 && skeleton {
				log.Info("===packet.Items() == 0 && skeleton===")
				skeleton = false
				getHeaders(from)
				continue
			}
			...
			if packet.Items() == 0 {
				log.Info("===packet.Items() == 0===")
				// Don't abort header fetches while the pivot is downloading
				if atomic.LoadInt32(&d.committed) == 0 && pivot <= from {
					p.log.Debug("No headers, waiting for pivot commit")
					select {
					case <-time.After(fsHeaderContCheck):
						log.Info("===time.After(fsHeaderContCheck)===")
						getHeaders(from)
						continue
					case <-d.cancelCh:
						return errCancelHeaderFetch
