		...
		if resultMsg {
			if msg.App == modules.APP_PAYMENT {
				...
				if !payment.IsCoinbase() && isSysContract {
					...
					redeemScript := tokenengine.Instalnce.GenerateRedeemScript(1, [][]byte{pubKey})
					...
					for inputIdx, input := range payment.Inputs {
						...
						sign, err := tokenengine.Instalnce.MultiSignOnePaymentInput(tx, tokenengine.SigHashAll, msgidx, inputIdx,
