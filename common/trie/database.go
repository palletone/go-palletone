...
type Database struct {
	diskdb ptndb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb ptndb.Database) *Database {
	return &Database{
		diskdb: diskdb,
...
func (db *Database) Commit(node common.Hash, report bool) error {
	...
	for hash, preimage := range db.preimages {
		...
		if batch.ValueSize() > ptndb.IdealBatchSize {
...
func (db *Database) commit(hash common.Hash, batch ptndb.Batch) error {
	// If the node does not exist, it's a previously committed node
