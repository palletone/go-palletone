...
func TestValidator(t *testing.T) {
	...
	tx.TxMessages = append(tx.TxMessages, &modules.Message{App: modules.APP_PAYMENT, Payload: &modules.PaymentPayload{Inputs: inputs, Outputs: outputs, LockTime: uint32(999)}},
		&modules.Message{App: modules.APP_DATA, Payload: &modules.DataPayload{FileHash: string("test text.")}}, &modules.Message{App: modules.APP_CONTRACT_TPL, Payload: &modules.ContractTplPayload{Name: "contract name"}})
	tx.Hash()
