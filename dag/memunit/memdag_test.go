...
func TestMemDag_AddUnit(t *testing.T) {
	...
	unitRep := dagcommon.NewUnitRepository(dagDb, idxDb, utxoDb, stateDb, propDb,tokenengine.Instalnce)
	...
	memdag := NewMemDag(gasToken, 2, false,
		db, unitRep, propRep, stateRep, cache(),tokenengine.Instalnce)
...
func TestMemDag_SwitchMainChain(t *testing.T) {
	...
	memdag := NewMemDag(gasToken, 2,
		false, db, unitRep, propRep, stateRep, cache(),tokenengine.Instalnce)
